...
class RecordsManageViews(APIView):
    ...
    def handle_back_data(self, serializerData):
        '''额外返回借用者姓名以及设备名'''
        data_tmp = serializerData
        user_name = All_users.objects.get(pk = data_tmp["b_user"]).get_user_name()
        device_name = All_devices.objects.get(pk = data_tmp["b_device"]).get_device_name()
        data_tmp["b_user_name"] = user_name
        data_tmp["b_device_name"] = device_name
        return data_tmp
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(self.handle_back_data(serializer.data))
        else:
            return Response(serializer.errors, status = status.HTTP_400_BAD_REQUEST)
