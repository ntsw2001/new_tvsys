...
class RecordsSerializer(serializers.Serializer):
    ...
    b_place = serializers.CharField(required = False, max_length = 50,
                                    error_messages = {"required":"设备所在地不能为空！", "max_length":"长度不能大于50！"})
    r_assign = serializers.DateTimeField(required = False, format='%Y-%m-%d %H:%M:%S')
    r_date = serializers.DateTimeField(required = False, format='%Y-%m-%d %H:%M:%S',
                                       error_messages = {"required":"实际归还时间不能为空！"})
    r_place = serializers.CharField(required = False, max_length = 50,
    ...
    def update(self, instance, validated_data):
        ...
        if validated_data.get('b_user'):
            instance.b_user = All_users.objects.get(pk = validated_data.get('b_user'))
        else:
            instance.b_user = validated_data.get(None, instance.b_user)

        if validated_data.get('b_device'):
            instance.b_device = All_devices.objects.get(pk = validated_data.get('b_device'))
        else:
